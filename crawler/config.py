...
crawler_config_mongo = Configure.from_path(
    pathlib.Path(os.path.dirname(__file__)) /'.env_mongo'
)

crawler_config_postgres = Configure.from_path(
    pathlib.Path(os.path.dirname(__file__)) /'.env_postgres'
)
